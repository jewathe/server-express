...
export const getGroceries = async (req, res) => {
    connect()
    query('SELECT DISTINCT r.id AS recipe_id, r.image AS recipe_image,r.proposed_title AS recipe_name,r.portions AS portions FROM groceries AS g INNER JOIN recipe AS r ON r.id=g.recipe_id INNER JOIN recipe_ingredients AS ri ON r.id=ri.recipe_id INNER JOIN ingredient AS i ON i.id=ri.ingredient_id', [], (resp) => {
